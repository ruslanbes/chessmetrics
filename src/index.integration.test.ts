...
// Create the Express app (same as in index.ts)
const app = express()
...
// Helper function to validate FEN format (same as in index.ts)
...
// Helper function to sanitize user input for error messages (same as in index.ts)
...
describe('Chessmetrics API Integration Tests', () => {
  ...
  describe('FEN Analysis Endpoint', () => {
    ...
    it('should analyze position after 1.e4 correctly', async () => {
      const fen = encodeURIComponent(CHESS_POSITIONS.AFTER_E4)
    ...
    it('should analyze position after 1.e4 e5 correctly', async () => {
      const fen = encodeURIComponent(CHESS_POSITIONS.AFTER_E4_E5)
      ...
      expect(response.body).toMatchObject({
        version: '1.0.0',
        fen: CHESS_POSITIONS.AFTER_E4_E5,
        gameType: 'standard',
        players: {
          white: {
            isMyTurn: true
    ...
    it('should handle invalid FEN with 400 error', async () => {
      const invalidFen = encodeURIComponent('invalid-fen')
      
      const response = await request(app)
        .get(`/api/v1/standard/fen/${invalidFen}`)
        .expect(400)

      expect(response.body).toMatchObject({
        error: {
          code: 'INVALID_FEN',
          message: 'Invalid FEN string format: invalid-fen'
        }
      })
      expect(response.body.timestamp).toBeDefined()
    })

    it('should handle malformed FEN with 400 error', async () => {
      const malformedFen = encodeURIComponent('rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR')
      
      const response = await request(app)
        .get(`/api/v1/standard/fen/${malformedFen}`)
        .expect(400)

      expect(response.body).toMatchObject({
        error: {
          code: 'INVALID_FEN',
          message: 'Invalid FEN string format: rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR'
        }
      })
    })

    it('should sanitize malicious input in error messages', async () => {
      const maliciousFen = encodeURIComponent('eval(alert("xss"))')
      
      const response = await request(app)
        .get(`/api/v1/standard/fen/${maliciousFen}`)
        .expect(400)

      expect(response.body).toMatchObject({
        error: {
          code: 'INVALID_FEN',
          message: 'Invalid FEN string format: xss'
        }
      })
    })

    it('should sanitize script tags in error messages', async () => {
      const scriptFen = encodeURIComponent('<script>alert("xss")</script>')
      
      const response = await request(app)
        .get(`/api/v1/standard/fen/${scriptFen}`)
        .expect(400)

      expect(response.body).toMatchObject({
        error: {
          code: 'INVALID_FEN',
          message: 'Invalid FEN string format: scriptxss/script'
        }
      })
    })
  })

  describe('PGN Analysis Endpoint', () => {
    it('should return coming soon message', async () => {
      const pgn = encodeURIComponent('1. e4 e5 2. Nf3 Nc6')
